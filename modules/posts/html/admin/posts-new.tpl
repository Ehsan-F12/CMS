...
function post_new()
{
	...
	$.ajax({
		type: 'post',
		url: '{admin-page}&module=posts&do=posts-new',
		data: $('#form-new-posts').serialize(),
		beforeSend: function()
		{
			apadana.loading(1);
		},
...
function post_reset()
{
    $('#div-ajax').html('');
    $('#tagBank').html('');
	apadana.$('form-new-posts').reset();
	$('#form-new-posts:hidden').slideDown('slow');
    $('#posts-tags').val('');
    $('#posts-tag').val('');
	CKEDITOR.instances.textarea_posts_text.setData('');
	CKEDITOR.instances.textarea_posts_more.setData('');[for editors]
    CKEDITOR.instances.{editor-id}.setData('');[/for editors]
	
	tags = new Array();
	tagIndex = 0;
	duplicateTagContorl = new Array();
...
<div id="div-ajax"></div>
<form id="form-new-posts" onsubmit="post_new();return false">

<!-- TAB START-->
<div class="content">
<div class="content-tabs">
<ul>
  <li class="tab-on" id="tab-id-1" onclick="apadana.changeTab(1, 3, function(){apadana.html('div-ajax', '')})">محتوای پست</li>
  <li class="tab-off" id="tab-id-2" onclick="apadana.changeTab(2, 3, function(){apadana.html('div-ajax', '')})">فیلدهای اضافی</li>
  <li class="tab-off" id="tab-id-3" onclick="apadana.changeTab(3, 3, function(){apadana.html('div-ajax', '')})">تنظیمات پست</li>
</ul>
</div>
...
<table cellpadding="6" cellspacing="0">
  <tr>
	...
	<td><input id="posts-title" name="posts[title]" value="" type="text" style="width:681px" lang="fa-ir" /></td>
  </tr>
  <tr>
	<td>نام مستعار</td>
	<td><input id="posts-name" name="posts[name]" value="" type="text" dir="ltr" style="width:700px" />&nbsp;<input type="button" value="بساز" onclick="post_slug('#posts-title', '#posts-name')" /></td>
  </tr>
  <tr>
  ...
  <tr>
	<td colspan="2" width="90"><input type="submit" value="ذخیره پست" />&nbsp;<input type="reset" value="از نو" onclick="post_reset()" /></td>
  </tr>
</table>
</div>
<!-- /option-id-1 -->
...
<table cellpadding="6" cellspacing="0">
  <tr>
	...
	<td><input id="posts-image" name="posts[image]" value="" type="text" dir="ltr" style="width:675px" />&nbsp;<input type="button" value="رسانه ها" style="width:65px" onclick="apadana.popupWindow('{admin-page}&section=media&noTemplate=true&input=posts-image', 'media', 1000, 700)" /></td>
  </tr>
  <tr>
	<td>موضوعات</td>
	<td>{categories}</td>
  ...
  <tr>
	<td>برچسب ها</td>
	<td>
	<input id="posts-tag" onkeypress="if (post_handleKeyPress(event)) {return false;} else return true;" type="text" /> <input type="button" name="save" value="اضافه کن" onclick="post_addTag();" />
	<br/><br/>
  ...
  <tr>
	<td></td>
	<td><input type="submit" value="ذخیره پست" />&nbsp;<input type="reset" value="از نو" onclick="post_reset()" /></td>
  </tr>
</table>
</div>
<!-- /option-id-3 -->
