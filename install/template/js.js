...
function install_check(action)
{
	switch (action)
	{
		case 'config':
			$.ajax({
				type: 'POST',
				url: 'install.php?action=config-check',
		...
		case 'admin':
			$.ajax({
				type: 'POST',
				url: 'install.php?action=admin-check',
...
function install_config()
{
	if (apadana.$('button-disabled').disabled != 'disabled')
	{
		$.ajax({
			type: 'POST',
			url: 'install.php?action=create-config',
			data: $('#form-config').serialize(),
			beforeSend: function()
			{
				apadana.loading(1);
...
function install_admin()
{
	if (apadana.$('button-disabled').disabled != 'disabled')
	{
		$.ajax({
			type: 'POST',
			url: 'install.php?action=admin-insert',
			data: $('#form-admin').serialize(),
			beforeSend: function()
			{
				apadana.loading(1);
